...
public:
    Load1(): Node("nnet_test")
    {
         ...
         timer_ = this->create_wall_timer(std::chrono::microseconds(1000000), std::bind(&Load1::timerCallback, this));
        //  timer_ = this->create_wall_timer(std::chrono::seconds(1), std::bind(&Load1::timerCallback, this));
        outputFile_.open("nnet_Enemy.csv");
...
private:
    ...
    void timerCallback()
    {
        ...
        if (outputFile_.is_open()) {
            ...
            std::cout << "Metrics saved in " << "nnet_Enemy.csv" << std::endl;
        } else {
